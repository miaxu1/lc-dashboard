...
export default function Dashboard(){
  ...
  const setSel=(key,updater)=>setSelectedAQs(p=>({...p,[key]:updater(p[key]??new Set())}));
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      const newSegs=[];
      const segConfig={
        "APD":  {label:"APD",         title:"APD — Loss Cost @ SIR",        hasExtra:true},
        "DM":   {label:"DM",          title:"DM — Loss Cost @ SIR",         hasExtra:true},
        "BI Combined":{label:"BI Combined",title:"BI Combined — Loss Cost @ SIR",hasExtra:true},
        "BI FL":{label:"BI FL",       title:"BI FL — Loss Cost @ SIR",      hasExtra:true},
        "UM":   {label:"UM SIR",      title:"UM — Loss Cost @ SIR",         hasExtra:true,  has250:true},
        "UM250":{label:"UM 250K",     title:"UM — Loss Cost @ 250K",        hasExtra:false},
      };

      for(const sheetName of wb.SheetNames){
        const ws=wb.Sheets[sheetName];
        if(sheetName==="UM"){
          // Check if 250K section exists
          const range=XLSX.utils.decode_range(ws["!ref"]);
          let has250=false, splitRow=-1;
          for(let r=range.s.r;r<=range.e.r;r++){
            const c0=ws[XLSX.utils.encode_cell({r,c:0})];
            if(c0?.v && String(c0.v).includes("250K")){has250=true;splitRow=r;break;}
          }
          // Parse SIR section
          const sirWS = has250 ? (() => {
            const newWS={...ws};
            // limit range to before 250k
            const r=XLSX.utils.decode_range(ws["!ref"]);
            r.e.r=splitRow-1;
            newWS["!ref"]=XLSX.utils.encode_range(r);
            return newWS;
          })() : ws;
          const {rows:sirRows,cq:sirCq}=parseSheet(sirWS,true);
          if(sirRows.length>0) newSegs.push({key:"UM",label:"UM SIR",title:"UM — Loss Cost @ SIR",cq:sirCq,hasExtra:true,data:sirRows});
          // Parse 250K section
          if(has250){
            const um250WS={...ws};
            const r=XLSX.utils.decode_range(ws["!ref"]);
            r.s.r=splitRow;
            um250WS["!ref"]=XLSX.utils.encode_range(r);
            const {rows:u250Rows}=parseSheet(um250WS,false);
            if(u250Rows.length>0) newSegs.push({key:"UM250",label:"UM 250K",title:"UM — Loss Cost @ 250K",cq:null,hasExtra:false,data:u250Rows});
          }
        } else {
          const cfg=segConfig[sheetName];
          if(!cfg) continue;
          const {rows,cq}=parseSheet(ws,cfg.hasExtra??true);
          if(rows.length>0){
            const key=sheetName.replace(/\s+/g,"_").toUpperCase().replace("COMBINED","").replace("_$","").trim();
            const segKey=sheetName==="APD"?"APD":sheetName==="DM"?"DM":sheetName==="BI Combined"?"BI":sheetName==="BI FL"?"BIFL":key;
            newSegs.push({key:segKey,label:cfg.label,title:cfg.title,cq,hasExtra:cfg.hasExtra??true,data:rows});
          }
        }
      }
      ...
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:10}}>
          ...
          <div
            ...
            style={{background:isDragging?"#1e2a3a":"#13132a",border:`2px dashed ${isDragging?"#5a7df5":"#2a2a4a"}`,
              borderRadius:8,padding:"8px 16px",cursor:"pointer",transition:"all 0.2s",
              display:"flex",alignItems:"center",gap:10,minWidth:220}}>
            <label style={{cursor:"pointer",display:"flex",alignItems:"center",gap:8,width:"100%"}}>
              <input type="file" accept=".xlsx,.xls" style={{display:"none"}}
                onChange={e=>{const f=e.target.files[0];if(f)handleFile(f);e.target.value="";}}/>
              <span style={{fontSize:18}}>📂</span>
              <div>
                <div style={{fontSize:12,fontWeight:600,color:uploadStatus==="success"?"#1D9E75":uploadStatus?.startsWith("error")?"#e24b4a":"#a0a0c0"}}>
                  {uploadStatus==="parsing"?"Parsing file...":uploadStatus==="success"?"✓ Data updated!":uploadStatus?.startsWith("error")?"⚠ "+uploadStatus.slice(6):"Upload LC_Triangle.xlsx"}
                </div>
                <div style={{fontSize:10,color:"#555"}}>Drag & drop or click to browse</div>
              </div>
            </label>
          </div>
        </div>
